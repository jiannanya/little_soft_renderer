...
public:
	...
	Rasterization(){}
	Rasterization(Vertex vertex):_vertex1(vertex){}
	Rasterization(Vertex vertex1, Vertex vertex2) :_vertex1(vertex1),_vertex2(vertex2) {}
	Rasterization(Vertex vertex1, Vertex vertex2, Vertex vertex3) :_vertex1(vertex1),_vertex2(vertex2),_vertex3(vertex3) {}
	Rasterization(std::vector<Vertex> vertexs):_vertexs(vertexs){}
...
void Rasterization::SetTextureData(Bitmap* bmp) {
	this->_textureData = bmp;
}


// 光栅化阶段插值函数
void Rasterization::Interpolation() {
...
三维空间三角形填充插值：
	...
	输出：
		三角形内的所有插值顶点数据
*/
std::vector<Vertex> Rasterization::TriangleInterpolation(std::vector<Vertex> vertexs) {
...
三维空间的线性插值：
	...
	输出：
		两个顶点间的所有插值顶点数据
*/
std::vector<Vertex> Rasterization::LinerInterpolation(Vertex v1, Vertex v2) {
...
glm::vec2 Rasterization::TexcoordCalc(glm::vec2 t1, glm::vec2 t2, float prop) {
	Color color;
...
// 由 y 算 x
int Rasterization::CalculateX(glm::vec4 point1, glm::vec4 point2, int y) {
...
// 由 x 算 y
int Rasterization::CalculateY(glm::vec4 point1, glm::vec4 point2, int x) {
...
int Rasterization::CalculateZ(glm::vec4 point1, glm::vec4 point2, int x, int y) {
...
float Rasterization::CalculateDistance(glm::vec4 point1, glm::vec4 point2) {
	return std::sqrt(std::pow(point1.x - point2.x, 2) + std::pow(point1.y - point2.y, 2) + std::pow(point1.z - point2.z, 2));
}

float Rasterization::CalculateProportion(glm::vec4 interPoint, glm::vec4 point1, glm::vec4 point2){
...
二维空间的线性插值
*/
std::vector<Vertex> Rasterization::LinerInterpolation2D(Vertex v1, Vertex v2) {
	std::vector<Vertex> vertexs;
	glm::vec4 point1 = v1.GetPosition();
	glm::vec4 point2 = v2.GetPosition();
	float k, b;
...
// 计算平面内直线斜率（线性）
void Rasterization::CalcSlopInPlane(glm::vec4 point1, glm::vec4 point2, float& k, float& b) {
...
// 计算插值点的Y值（线性）
int Rasterization::CalcY(int x, float k, float b) {
	return (int)k * x + b;
}

// 计算插值点的Z值（线性）
int Rasterization::CalcZ(int x, int y, glm::vec4 leftPoint, glm::vec4 rightPoint) {
...
// 计算平面内两点距离（线性）
float Rasterization::CalcDistance(glm::vec2 point1, glm::vec2 point2) {
	return std::sqrt(std::pow(point1.x - point2.x, 2) + std::pow(point1.y - point2.y, 2));
}

// 计算平面内插值比例（线性）
float Rasterization::CalcInterProportion(glm::vec4 interPoint, glm::vec4 point1, glm::vec4 point2) {
...
// 按三角形三个顶点的x坐标排序
std::vector<Vertex> Rasterization::SortTriangleVertex(std::vector<Vertex> vertexs) {
...
计算得到三角形边上的vertex
	...
	输出：
		边上某点的vertex
*/
Vertex Rasterization::GetInterVertexInLine(Vertex vertex1, Vertex vertex2, int x) {
...
// 法向量插值
glm::vec3 Rasterization::NormalCalc(Vertex v1, Vertex v2, float prop) {
...
// 颜色插值
glm::vec4 Rasterization::LinerColorInterpolationCalc(glm::vec4 leftColor, glm::vec4 rightColor, float proportion) {
	glm::vec4 interColor;
...
std::vector<Vertex> Rasterization::GetResVertexs() {
